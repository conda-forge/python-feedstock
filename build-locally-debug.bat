...
conda-build -m ..\..\a\conda_build_config-dbg.yaml . --python 3.8 --no-build-id -c rdonnelly --keep-old-work --dirty %CB_DEBUG% 2>&1 | C:\msys32\usr\bin\tee %CONDA_PREFIX%\conda-bld\lief\build.log
...
mkdir %CONDA_PREFIX%\conda-bld\lief
conda-build -m ..\..\a\conda_build_config.yaml . --python 3.8 --no-build-id -c rdonnelly --keep-old-work --dirty %CB_DEBUG% 2>&1 | C:\msys32\usr\bin\tee %CONDA_PREFIX%\conda-bld\lief\build.log
...
mkdir %CONDA_PREFIX%\conda-bld\lief
conda-build -m ..\..\a\conda_build_config.yaml . --python 3.7 --no-build-id -c rdonnelly --keep-old-work --dirty %CB_DEBUG% 2>&1 | C:\msys32\usr\bin\tee %CONDA_PREFIX%\conda-bld\lief\build.log
