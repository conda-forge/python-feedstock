...
if %_BS_THREE_EIGHT%==yes (
  ...
  if %_BS_RELEASE_ME%==yes (
    if "%_BS_SKIP_WILDCARD%"=="" or not dir %_BS_CB_CROOT%\%_BS_SKIP_WILDCARD_PREFIX%.tar.bz2 /b /a-d >nul 2>&1 (
      ...
      del /s /q !_BS_DST_DIR!
      mkdir !_BS_DST_DIR!
      echo conda-build --croot %_BS_CB_CROOT% --build-id-pat {n}-3.8.1-%PF% -m %DBG_CFG% . --python 3.8 %CHANNELS% %CB_KEEP% %CB_DEBUG% | C:\msys32\usr\bin\tee !_BS_DST_DIR!\build.log
      conda-build --croot %_BS_CB_CROOT% --build-id-pat {n}-3.8.1-%PF% -m %DBG_CFG% . --python 3.8 %CHANNELS% %CB_KEEP% %CB_DEBUG% 2>&1 | C:\msys32\usr\bin\tee -a !_BS_DST_DIR!\build.log
    )
  )
)
...
if %_BS_THREE_SEVEN%==yes (
  ...
  if %_BS_RELEASE_ME%==yes (
    if "%_BS_SKIP_WILDCARD%"=="" or not dir %_BS_CB_CROOT%\%_BS_SKIP_WILDCARD_PREFIX%.tar.bz2 /b /a-d >nul 2>&1 (
      ...
      del /s /q !_BS_DST_DIR!
      mkdir !_BS_DST_DIR!
      echo conda-build --croot %_BS_CB_CROOT% --build-id-pat {n}-3.7.6-%PF% -m %DBG_CFG% . --python 3.7 %CHANNELS% %CB_KEEP% %CB_DEBUG% | C:\msys32\usr\bin\tee !_BS_DST_DIR!\build.log
      conda-build --croot %_BS_CB_CROOT% --build-id-pat {n}-3.7.6-%PF% -m %DBG_CFG% . --python 3.7 %CHANNELS% %CB_KEEP% %CB_DEBUG% 2>&1 | C:\msys32\usr\bin\tee -a !_BS_DST_DIR!\build.log
    )
  )
)
